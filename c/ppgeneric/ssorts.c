...
void printUsage() {
	...
	printf("\t-I InsertionSort (just one kind, and beware it is slow for big stuff)\n");
	printf("\t-K BlockSort (TBD).\n");
	printf("\t-L LibrarySort (TBD).\n");
	printf("\t-M MergeSort ([%s],%s,%s).\n",HPS2MergeSort,WPBottomUpMergeSort,XCTopDownMergeSort);
...
void handleArgs(int argc, char * const* argv, char *cmpflag, FILE **ifp, FILE **ofp, char *st, bool *v) {
	...
	*v = false;
	...
	while ((c = getopt (argc, argv, "B:C:G:H:IK:L:M:N:O:P:Q:R:S:T:U:V:c:dhi:n:o:s:v")) != -1)
		switch (c) {
			case 'B': /* BubbleSort */
...
void manageSort(char cmpFlag, char *sortType, FILE *iFp, bool verbose, FILE *oFp) {
	...
	if ( u && strcmp(sortType,HPS2MergeSort) == 0 )			{ u=false; hps2MergeSort(itemlist,itemcount,cmp); }
	...
	if ( u && strcmp(sortType,WPBottomUpMergeSort) == 0 )	{ u=false; wpBottomUpMergeSort(itemlist,itemcount,cmp); }
	if ( u && strcmp(sortType,WPPCInsertionSort) == 0 )		{ u=false; wppcInsertionSort(itemlist,itemcount,cmp); }
	...
	if ( u && strcmp(sortType,XCTopDownMergeSort) == 0 )	{ u=false; xcTopDownMergeSort(itemlist,itemcount,cmp); }
